/*
    ...
    - DBMS : DataBase Managerment System의 약자, 데이터베이스 관리 시스템
     Create : 추가
     Read : 보기
     Update : 수정
     Delete : 삭제
...
/*
	BETWEEN AND
		- 조건식에서 사용되는 구문
        - 몇 이상 몇 이하인 범위에 대한 조건을 제시할 때 사용하는 연산자
        
		비교대상컬럼 BETWEEN 하한값 AND 상향값
*/
-- 급여가 350만원 이상 600만원 이하를 받는 사원들의 사원명, 사번, 급여 조회
SELECT emp_name, emp_id, salary
FROM employee
...
-- 이름 가운데 글자가 '하'인 사원들의 사원명, 전화번호 조회 (이름이 3글자)
...
*/
-- 보너스를 받지 않는 사원(bonus 값이 null)들의 사번(emp_id), 이름, 급여, 보너스 조회
...
-- 3. 연봉(보너스포함X)이 4000만원 이상이고 보너스를 받지 않은 사원들의 사번(emp_id), 사원명, 급여, 보너스(bonus) 조회
SELECT emp_id, emp_name, salary, bonus
FROM employee
...
-- 6. 보너스 포함연봉이 NULL이 아니고 이름에 '하'가 포함되어 있는 사원들의 사번, 사원명, 급여, 보너스 포함 연봉(별칭 부여) 조회 (연봉이 높은 순으로 정렬)
...
AND emp_name LIKE '%하%'
ORDER BY 연봉 DESC; -- 별칭으로 정렬 가능 (" "가 들어간 별칭은 정렬 불가)
